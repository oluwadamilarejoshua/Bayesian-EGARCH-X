// Robustness variant: EXCLUDE June & July 2016 from the likelihood.
// The recursive variance path (epsilon, log_h, h) is computed exactly
// as in the baseline model, using the true data at every t -- so the
...
// Purpose: test whether betaDebt survives without the two months whose
// debt-indicator (GS10) value happens to coincide with the naira's
// largest shocks in the sample (see diagnostic write-up).
...
model {
  ...
  for (t in 1:T)
    target += weight[t] * normal_lpdf(epsilon[t] | 0, sqrt(h[t]));
}
