// Prior sensitivity variant B: FLAT / UNINFORMATIVE
// betaGDP and betaDebt centred at 0 with a very wide SD -- no directional
// signal at all, i.e. "let the data decide" with (almost) no help from
// theory. If the baseline posterior survives this, the data is doing
// the real work, not the citations.
...
model {
  ...
  betaGDP  ~ normal( 0.00, 2.00);   // <-- flat: no sign, no magnitude info
  betaDebt ~ normal( 0.00, 2.00);   // <-- flat: no sign, no magnitude info
