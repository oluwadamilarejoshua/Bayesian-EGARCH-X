...
1. **`01_data_fetch_monthly.R`** — Downloads the monthly official NGN/USD exchange rate (IMF IFS via `rdbnomics`, with a FRED fallback) and the annual WDI GDP growth and debt-service series (via the `WDI` API). Disaggregates the annual GDP/debt series to monthly frequency using the **Denton-Cholette** temporal disaggregation method (`tempdisagg`, Denton 1971; Sax & Steiner 2013) *with a genuine monthly indicator series for each* — the WTI oil price for GDP (Nigeria's output is heavily oil-driven) and the US 10-Year Treasury yield for debt service (most of Nigeria's external debt is dollar-denominated, so global rates drive its servicing cost, and the series is fully exogenous to the naira market). An earlier version ran Denton-Cholette with no indicator at all (`~ 1`), which just produces the smoothest mathematically possible path with no real monthly signal behind it — this was replaced because it left the interpolated series unrealistically smooth and, as a side effect, inflated their apparent autocorrelation (see step 3). A reserves-based debt indicator was tried in between and also dropped, since Nigeria's FX reserves are the CBN's own instrument for defending the naira band — using them would let the debt regressor share a common driver with the dependent variable. Writes the full 1980-2020 merged series and a provisional post-1999 subset (superseded by step 2) to `Data/processed/` (created automatically).
